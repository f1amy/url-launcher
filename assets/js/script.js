...
document.getElementById("urls-button").onclick = function() {
    ...
    chrome.storage.sync.get("URLs", function(urlsObject) {
        if (urlsObject["URLs"] !== undefined && urlsObject["URLs"].length > 0) {
            // TODO: ^ check, refactor
            for (const currentUrl of urlsObject["URLs"]) {
                if (
                    Array.from(
                        document.querySelectorAll("#urls-page #url-list li a")
  ...
  chrome.storage.sync.get("URLs", function(urlsObject) {
    if (urlsObject["URLs"] !== undefined && urlsObject["URLs"].length > 0) {
      ...
      if (urlsObject["URLs"].indexOf(url.href) === -1) {
        ...
        createAlert("URL was successfully added!", alertTypeEnum.SUCCESS);
      } else {
        createAlert("URL has already been added!", alertTypeEnum.DANGER);
        return;
      }
    } else {
      urlsObject = {
  ...
  chrome.storage.sync.get("URLs", function(urlsObject) {
    ...
    for (const currentBox of checkedBoxes) {
      urlsObject["URLs"].splice(
        urlsObject["URLs"].indexOf(currentBox.nextElementSibling.href),
        1
      );
    ...
    createAlert("URLs have been successfully deleted!", alertTypeEnum.SUCCESS);
  });
};
    ...
    chrome.storage.sync.get("URLs", function(urlsObject) {
      if (urlsObject["URLs"] !== undefined && urlsObject["URLs"].length > 0) {
        ...
        for (const currentUrl of urlsObject["URLs"]) {
          if (
            Array.from(
              document.querySelectorAll("#urls-list-tab-content #url-list li a")
            ).every(a => a.href !== currentUrl)
          ) {
            let li = document.createElement("li");
